 ...
 * Fetches the most recent MESSAGE_LIMIT messages from the channel and formats them into an array of AIChatItem objects.
...
async function createChatHistory(channel: PublicThreadChannel<boolean>): Promise<AIChatItem[]> {
    const messages = await channel.messages.fetch({ limit: MESSAGE_LIMIT });
    const chatHistory = [];
